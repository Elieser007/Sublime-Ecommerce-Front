...
- **No comments in code** unless critical (XSS prevention, CSS scoping notes). Exception: `variant-selector.js` keeps its component-level contract docs (header, section dividers) and design-traceability comments such as the D1 renderer-reuse note in the `VariedadSelector` case — that Web Component predates this rule and its comments are part of the SDD design record (change `caramelos-variedad`). Same exception for `product-card.js` and `variant-modal.js`: their component-contract headers and design-record comments (SSG bake/bake-outcome wiring, modal-stack/back parity, availability filtering) predate this rule and are load-bearing contracts of the same Web Component family, so they stay documented. Same exception for the SSG catalog modules created by change `ssg-catalog-batch`: `src/lib/catalog-batch.ts`, `src/lib/catalog-build.ts`, `src/lib/variant-fallback.ts`, `src/lib/variant-filter.ts`, `src/lib/variant-logic.ts` keep their header doc blocks and JSDoc as the design record (pipeline guarantees, the bake-outcome signal rule, the SQLite aggregate-ORDER-BY invariant, and the availability/parity semantics), because those contracts are load-bearing for the SSG build and must stay documented. Same exception for `src/lib/gallery-lightbox.ts` (created by change `product-gallery-lightbox`): its header doc block and per-function JSDoc are the design record for the locked zoom/pan/navigation conventions (REQ-10..17, cursor-anchor math with stage-center origin, wheel direction rules), which must stay documented so future edits cannot silently flip them. Same exception for `number-input.js` and `src/lib/qty-width.ts` (created by change `quantity-9999-dynamic-width`): their component-contract header/JSDoc document the shared quantity-stepper contract (data-* attributes, `number-input:change` event, clamp semantics, dynamic digit-driven width) and the digit-width formula's fails-wide `String(value).length` semantics, which are load-bearing for the quantity UI family and must stay documented.
