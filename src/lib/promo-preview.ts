...
function imageAttr(imageUrl: string): string {
  return escapeHtml(getProductImageUrl(imageUrl));
}

function buildHero(first?: PromoPreview): string {
  if (!first) return "";
  return `<div class="hero-promo"><div class="hero-bg" style="background-image:url('${imageAttr(first.imageUrl)}')"></div><div class="hero-overlay"></div><div class="hero-content">${first.title ? `<h2 class="hero-title">${escapeHtml(first.title)}</h2>` : ""}${first.subtitle ? `<p class="hero-desc">${escapeHtml(first.subtitle)}</p>` : ""}<a href="${escapeHtml(sanitizePromoUrl(first.link))}" class="hero-btn">Ver más</a></div></div>`;
}
