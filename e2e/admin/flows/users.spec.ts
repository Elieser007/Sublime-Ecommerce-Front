 ...
 * seeded users (user-client-e2e + admin@sublime.com), create via the users
 * modal (password + branch assignment — the GET /api/users list is scoped to
 * the admin's branches, so a user without a branch would never appear),
 * edit name/role, hard delete (🗑️ + native confirm), and avatar upload on
 * create (file input → client-side canvas processing → POST /api/upload →
 * persisted avatarUrl). All mutations run against users created by the flow
 * itself; the seeded fixtures are asserted by email only (observation 19:
 * the admin may be a pre-existing dev admin, never assert user-admin-e2e).
 */
...
test("lists seeded users and filters by role", async ({ page }) => {
  ...
  // Search matches name or email (300ms debounce; expect auto-waits).
  await page.locator("#search").fill(SEEDED_CLIENT_EMAIL);
  const row = page.locator("tbody tr", { hasText: SEEDED_CLIENT_EMAIL });
  await expect(row).toBeVisible();
  await expect(row).toContainText(SEEDED_CLIENT_NAME);
  ...
  // Seeded admin (current session) listed with the admin role and self tag.
  await page.locator("#search").fill(SEEDED_ADMIN_EMAIL);
  await page.locator("#role-filter").selectOption("");
  const adminRow = page.locator("tbody tr", { hasText: SEEDED_ADMIN_EMAIL });
  await expect(adminRow).toBeVisible();
  await expect(adminRow).toContainText("Admin");
  await expect(adminRow).toContainText("Tú");
});
...
async function createUser(page: Page, name: string, email: string): Promise<void> {
  ...
  // Branch assignment is required for the created user to show up: the list
  // is scoped to the admin's branches (GET /api/users).
  await modal.locator("#branch-ids").selectOption({ label: "Principal" });
